...
/**
 * POST /api/relationships/activities/solo
  ...
  async (req: AuthRequest, res: Response<ApiResponse<{ player: any }>>) => {
    ...
    try {
      ...
      if (activity.requiresNPC) {
        res.status(400).json({
          success: false,
          error: 'This activity requires a neighbor'
        });
        return;
      }
      ...
      console.log(`✅ User ${userId} performed solo activity: ${activity.name}`);
      ...
      console.error(`❌ Error performing solo activity for user ${userId}:`, error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to perform activity';
      res.status(500).json({
        success: false,
        error: errorMessage
